...
app.get('/orders/getOrder/:orderId', async (req, res) => {
    ...
    try{
        const customer = await axios.get(`http://localhost:3002/customers/getCustomer/${orderData.customerId}`);
        const product = await axios.get(`http://localhost:3001/products/getProduct/${orderData.productId}`);
